...
void sendMatrixToShiftRegister(uint8_t row)
{
	...
	_delay_us(DELAYSPI);
	usartReceiveTransmit(actualMatrix[row].low | rowMaskHigh); // only a OR operation
	_delay_us(DELAYSPI);
...
void actualizeMatrixWithSystemTime()
{
		...
		if ((systemTime.minute >=58 && systemTime.minute <2)  ||
			(systemTime.minute >=13 && systemTime.minute <17) ||
			(systemTime.minute >=28 && systemTime.minute <32) ||
			(systemTime.minute >=43 && systemTime.minute <47))
		{
		...
		// past
		if ((systemTime.minute >=2 && systemTime.minute <8)   ||
			(systemTime.minute >=17 && systemTime.minute <23) ||
			(systemTime.minute >=32 && systemTime.minute <38) ||
			(systemTime.minute >=47 && systemTime.minute <53))
		{
		...
		// to
		if ((systemTime.minute >=8 && systemTime.minute <13)  ||
			(systemTime.minute >=38 && systemTime.minute <43) ||
			(systemTime.minute >=53 && systemTime.minute <58))
		{
		...
		// to and half
		if (systemTime.minute >=24 && systemTime.minute <28)
		{
		...
		// half
		if (systemTime.minute >=28 && systemTime.minute <38)
		{
		...
		// quarter past
		if (systemTime.minute >=8 && systemTime.minute <23)
		{
		...
		// quarter to
		if (systemTime.minute >=38 && systemTime.minute <53)
		{
			actualMatrix[3].high	= WORD_ROW03_VAEDDL_H | WORD_ROW03_DREI_H;
			actualMatrix[3].low		= WORD_ROW03_VAEDDL_L | WORD_ROW03_DREI_L;
		}
